...
class MetacognitiveController:
    def __init__(self, neural_threshold=0.9, symbolic_threshold=0.7):  # Adjusted neural threshold
...
class SyntheticMedicalDataset:
    def __init__(self, num_samples=40, num_features=10, num_classes=4, random_state=42):  # Increased num_samples
    ...
    def _generate_data(self):
        ...
        # Balanced class distribution: 10 samples per class
        class_assignments = [0] * 10 + [1] * 10 + [2] * 10 + [3] * 10
...
if __name__ == "__main__":
    ...
    # Initialize dataset with more samples
    dataset = SyntheticMedicalDataset(num_samples=40, num_features=input_dim, num_classes=num_classes)

    # Split into train (75%) and test (25%)
    indices = list(range(dataset.num_samples))
    np.random.shuffle(indices)
    split = int(0.75 * len(indices))  # 30 train, 10 test
    ...
    # Initialize knowledge graph with refined rules
    kg = nexus.init_knowledge_graph()
    kg.add_relation(0, 'indicates', 1, 0.8)  # fever -> cough
    kg.add_relation(5, 'strongly_indicates', 3, 0.90)  # shortness_of_breath -> pneumonia (reduced slightly)
    kg.add_rule([0, 1], 1, 0.9)  # IF fever AND cough THEN Flu
    kg.add_rule([5, 0], 3, 0.90)  # IF shortness_of_breath AND fever THEN Pneumonia (reduced slightly)
    kg.add_rule([8, 5], 2, 0.95)  # IF loss_of_taste AND shortness_of_breath THEN COVID (increased)
    kg.add_rule([2], 0, 0.85)  # IF runny_nose THEN Common Cold
    kg.add_rule([2, 1], 0, 0.90)  # IF runny_nose AND cough THEN Common Cold (new rule)

    # Define symbol-to-class mapping with adjusted weights
    symbol_to_class_dict = {
        ...
        5: {2: 0.85, 3: 0.90},        # shortness_of_breath -> COVID, Pneumonia (Pneumonia reduced)
        8: {2: 0.95}                  # loss_of_taste -> COVID (increased)
    }
    ...
    print("This section compares Neural, Symbolic, and NEXUS predictions for the 10 test patients.")
    ...
    for i, (inputs, labels) in enumerate(test_loader):
        test_idx = test_indices[i]  # Map to original dataset index
        try:
    ...
    print("\nInsights:")
    print("- Neural: Relies on pattern recognition from training data. Tested for generalization on unseen test set.")
    print("- Symbolic: Enhanced rules improve differentiation (e.g., COVID vs. Pneumonia) and handle combinations.")
    print("- NEXUS: Adjusted thresholds allow symbolic influence, aiming to balance data-driven and rule-based reasoning.")
    print("This updated model tests generalization and hybrid potential with a larger dataset and refined logic.")